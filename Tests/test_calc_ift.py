...
def test_denss(gi_gnom_ift, temp_directory):
    ...
    assert np.allclose(chi_sq, 0.8780439375606036)
    assert np.allclose(rg, 34.1149940995528)
    assert np.allclose(support_vol, 415132.57150268555)
    assert np.allclose(I_fit.sum(), 0.1412691428231155)
...
def test_denss_average(temp_directory):
    ...
    assert np.isclose(mean_cor, 0.9543698452921, rtol=1e-3)
    assert np.isclose(std_cor, 0.004750079261040415, rtol=1e-1)
    assert res == 26.1
...
def test_denss_align(temp_directory):
    ...
    assert np.isclose(score, 0.8434734715650835)
