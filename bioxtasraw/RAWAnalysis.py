...
class UVConcentrationPanel(wx.Panel):
    ...
    def __init__(self, parent, name, selected_sasms = None, bg_sasm = None):
        ...
        self.double_spins = ['UVDarkTransmission', 'UVTransmissionBg', 'UVTransmissionSamp', 'UVPathlength', 'UVExtinctionCoeff']
    ...
    def setSampleValues(self, sasm):
        ...
        #If we only have 1 selected sasm, then you can change the sample transmission, otherwise it will just use the header value
        if len(self.selected_sasms) > 1: 
           self.spin_ctrl_ids['UVTransmissionSamp'][3]=float(samp_data['UVTransmission'])
    ...
    def _initLayout(self):
        ...
        for key in self.all_spins:
            ...
            if key in self.double_spins:
                spin_ctrl = wx.SpinCtrlDouble(self, spin_id, value = '1.0', min = 0.0, max = 16000.0, initial = 1.0)
                spin_ctrl.SetIncrement(0.1)
                spin_ctrl.SetDigits(2)
                spin_ctrl.Bind(wx.EVT_SPINCTRLDOUBLE, self.onSpinUpdate)
                if key in ['UVTransmissionSamp']:
                    if len(self.selected_sasms) > 1:
                       spin_ctrl.Enable(False)
            else:    
                spin_ctrl = wx.SpinCtrl(self, spin_id, value = '1', min = 0.0, max = 16000, initial = 1)
